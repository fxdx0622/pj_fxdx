...
[Serializable, VolumeComponentMenu("Post-processing/Custom Depth Of Field")]
...
public class CustomDepthOfField : VolumeComponent, IPostProcessComponent
{
	...
	{
		if (mode.value == CustomDepthOfFieldMode.Off || SystemInfo.graphicsShaderLevel < 35)
			return false;

		return mode.value != CustomDepthOfFieldMode.Gaussian || SystemInfo.supportedRenderTargetCount > 1;
	}
...
/// <summary>
/// A <see cref="VolumeParameter"/> that holds a <see cref="CustomDepthOfFieldMode"/> value.
/// </summary>
[Serializable]
